...
const records = ref<{ match: string; time: string; profit: string; status: string }[]>([])
...
onMounted(() => {
  ...
  projectId.value = order?.projectId || 0
  if (projectId.value) loadRecords()
})
...
async function loadRecords() {
  ...
  page.value = 1
  hasMore.value = true
  const res = await ProjectApi.getTradeLogs(projectId.value, page.value)
  if (res.code === 0 && res.data) {
    const data = res.data
    const list: any[] = Array.isArray(data) ? data : (data.data || [])
    records.value = list.map(parseRecord)
...
async function loadMore() {
  ...
  if (res.code === 0 && res.data) {
    const data = res.data
    const list: any[] = Array.isArray(data) ? data : (data.data || [])
    records.value.push(...list.map(parseRecord))
    hasMore.value = (parseInt(data.total_page?.toString() || '0') || 0) > page.value
  }
  loadingMore.value = false
}

function parseRecord(m: any) {
  const profit = parseFloat(m.actual_profit?.toString() || '0') || 0
  return {
    match: m.event_name?.toString() || '',
    time: m.created_time || '',
    profit: `${profit >= 0 ? '+' : ''}${profit.toFixed(2)} USDT`,
    status: (m.status?.toString() === '2' || m.phase?.toString() === '3') ? t('completed') : t('running'),
  }
}
</script>
    ...
    <div class="records-content">
      ...
      <div v-else class="record-list" ref="listRef">
        <div v-for="(r, i) in records" :key="i" class="record-row">
          ...
          <div class="row-right">
            <strong :class="{ positive: !r.profit.startsWith('-') }">{{ r.profit }}</strong>
            <span class="status-tag" :class="{ completed: r.status === t('completed') }">{{ r.status }}</span>
          </div>
        </div>
...
.record-row {
  ...
  .row-right {
    ...
    .status-tag {
      ...
      font-size: 11px; font-weight: 600; color: #FFB800; background: rgba(#FFB800, 0.12);
      &.completed { color: $green; background: rgba($green, 0.12); }
    }
  }
}
...
.no-more {
  text-align: center; padding: 16px; color: $muted; font-size: 12px;
}
