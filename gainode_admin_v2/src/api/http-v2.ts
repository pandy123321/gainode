/**
 * Admin V2 统一 HTTP 客户端 —— 六请求头 + auth refresh single-flight + RESULT_UNKNOWN。
 * 对齐 05 §1（写幂等/乐观锁）、05 §7（16 项错误码）、05 §2.1（/api/v1/auth/refresh）。
 * 与 V1.x `api/http.ts`（MD5 签名 + code/msg/data）并存，过渡期新页走本 client，
 * Layui 旧页保持旧 client 直至逐页迁移完成。
 *
 * 关键约束（07 §9 S04 / 05 §7）：
 * - 写请求超时/无响应 → RESULT_UNKNOWN，不得自动重试创建，需用原 Idempotency-Key 查询。
 * - object_version 经 If-Match 透传；冲突由后端返回 OBJECT_VERSION_CONFLICT(409)。
 */
import axios, {
  AxiosError,
  AxiosHeaders,
...
// ---- 令牌/语言持有器（由登录/session store 写入，避免与 store 循环依赖） ----
let accessToken: string | null = null
let refreshToken: string | null = null
let languageGetter: () => string = () => 'zh_CN'
...
export async function request<T>(config: GainodeRequestConfig): Promise<Envelope<T>> {
