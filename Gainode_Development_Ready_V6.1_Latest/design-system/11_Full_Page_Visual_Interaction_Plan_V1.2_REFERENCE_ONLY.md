# Gainode 全量页面视觉与交互策划 V1.2 · REFERENCE_ONLY

> **DOCUMENT_STATUS = REFERENCE_ONLY**  
> **FORMER_ROLE = MERGE_SOURCE**  
> **SOURCE_OF_TRUTH = 01–08 + /i18n assets + Logo**  
> 本文档不再与 03/04/08 并行维护。V1.1 中已确认的逐页视觉/交互补充已合并到 `03/04 V2.2`；I18N 全局规则已合并到 `08 V2.2`。以后原型 Agent 不得只读取本文档执行 HIFI。
...
01 产品功能基线
02 经济模型与业务规则
03 Mobile/H5 每页最终 HIFI Page Execution Spec
04 Admin 每页最终 HIFI Page Execution Spec
05 数据 / 状态 / 权限 / API
06 参数
07 开发与验收
08 全局视觉 + I18N/L10N
...
本文档 V1.2 = REFERENCE_ONLY（只保留策划背景、设计 rationale 与合并追溯）
```

**冲突处理：** 页面尺寸/布局/组件/状态出现冲突时，Mobile/H5 以 03 为准，Admin 以 04 为准；全局颜色/Logo/字体/响应式/I18N 以 08 为准；业务字段/权限/API 以 05/06 为准。
...
# 14. 多语言与国际化策划追溯（规则已合并至 08 V2.2）
支持语言仍为 `zh-CN / en-US / ja-JP / ko-KR / th-TH / de-DE / fr-FR`。最终字符串不再以内嵌表格作为权威来源，而读取 `/i18n/*.json`；75 页 Key 映射读取 `/i18n/ui-copy-manifest.json`。
## 14.1 全局原则
...
> **V1.2 说明：** V1.1 的内嵌七语字典与页面标题翻译表已移除；最终字符串只读取 `/i18n/*.json`。下面仅保留逐页本地化行为矩阵作为设计追溯。

## 14.2 字符串资产（权威来源已迁移）

V1.1 内嵌的“核心 UI 七语字典”和“75 个页面标题七语映射”已停止维护，避免 Markdown 表格与工程字符串发生漂移。
...
其中 `ui-copy-manifest.json` 明确 75 个 Page ID 使用的 title / description / primary action / state / domain-sensitive keys；任何新增用户可见文案必须先进入 JSON。
...
## 14.4 多语言 Gate（V1.2 收口状态）

```text
LANGUAGE_COUNT = 7
PAGE_ID_COVERAGE = 75/75
I18N_KEY_COVERAGE = 100%  # against ui-copy-manifest.json
MISSING_KEY = 0
MANIFEST_REFERENCE_MISSING = 0
RAW_ENUM_VISIBLE = 0  # implementation gate; prototype/code must verify
...
因此本文件不再自称“七语言最终稿”。AI 已提供完整工程字符串草稿与覆盖资产，但敏感文案必须由授权人工 Owner 签核。

```text
READY_FOR_HIFI_STRUCTURE = YES
READY_FOR_CHINESE_HIFI = YES
READY_FOR_INTERNAL_7_LANGUAGE_LAYOUT_TEST = YES
READY_FOR_FINAL_7_LANGUAGE_HIFI = NO  # 等待敏感文案人工签核
```

人工签核后，且实现侧通过 `MISSING_KEY=0 / RAW_ENUM_VISIBLE=0 / 7 locale layout` 回归，才允许改为：

```text
READY_FOR_FINAL_7_LANGUAGE_HIFI = YES
```
