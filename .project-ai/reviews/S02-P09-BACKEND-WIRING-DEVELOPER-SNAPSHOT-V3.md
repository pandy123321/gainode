...
Open issues / Risk / Not implemented:
  ...
  - 【更新】Admin V2 后端接口累计 17 个 + 前端 14 权威页真实数据：后端新增审计详情（/api/v1/admin/audit-log/{id}）与资产总览（/api/v1/admin/ledger/overview）；前端 admin-v2.ts + pageData.ts 接入 14 权威页 loader。wiring 契约测试扩至 102 断言全绿，vite build 通过。
  - 【更新】Admin V2 后端接口累计 18 个 + 前端 15 权威页真实数据：后端新增 APT 流水明细（/api/v1/admin/ledger/entries）；前端接入 /ledger/entries loader。wiring 契约测试扩至 104 断言全绿，vite build 通过。
