 ...
 * @author Kevin
 */
class Dao implements MakeInterface
{
    /**
     * 获取数据连接对象
     * @var Db
    ...
    public function getTemplate(string $name) {
        if(empty($this->content)){
            $this->content = file_get_contents(resource_path("template/make/TemplateDao.php"));
        }
        $data = explode('_',$name);
        $module = array_shift($data);
        $model = '';
