...
class BscTransferSubscribe
{
    ...
    public function __construct(callable $callback, array $override = [])
    {
        ...
        $contract = $override['contract'] ?? config('web3.listen_token_contract');
        $decimals = (int)($override['decimals'] ?? config('web3.listen_token_decimals', 18));
        $symbol   = $override['symbol']   ?? config('web3.listen_token_symbol', 'USDT');
        $network  = $override['network']  ?? config('web3.listen_token_network', 'BEP20');
        $wss      = $override['wss_url']  ?? (config('web3.wss_url') . '/' . config('web3.rpc_key'));
        ...
        $this->config = [
            ...
            'symbol'   => $symbol,
            'network'  => $network,
            'required_confirmations' => (int)config('web3.listen_required_confirmations', 6),
            'reconnect_delay' => 5,
            'historical' => [
                'enabled'   => true,
                'from_block' => config('web3.listen_from_block', 'latest'),
                'to_block'   => 'latest',
            ],
        ];
    ...
    public function refreshAddressMap(): void
    {
        try {
            ...
            $this->addressMap = $walletService->getAddressUserMap('bsc');
            Log::info("[BscTransfer] 系统地址映射已加载，共 " . count($this->addressMap) . " 个");
        } catch (\Throwable $e) {
            Log::error("[BscTransfer] 加载系统地址映射失败: " . $e->getMessage());
        }
    }
    ...
    protected function connect(): void
    {
        ...
        if (!$urlInfo || !isset($urlInfo['host'])) {
            Log::error("[BscTransfer] 无效的WebSocket URL: {$this->config['wss_url']}");
        ...
        Log::info("[BscTransfer] 正在连接 {$host}:{$port} ...");
    }

    public function onConnect(AsyncTcpConnection $connection): void
    {
        Log::info("[BscTransfer] 已连接到 BSC 节点");
        $this->rpcId++;
        $request = [
            'jsonrpc' => '2.0',
            'id'      => $this->rpcId,
    ...
    public function onMessage(AsyncTcpConnection $connection, string $data): void
    {
        try {
            ...
            Log::error("[BscTransfer] 无效JSON响应: " . $e->getMessage());
            return;
        }
        ...
        if (isset($response['error'])) {
            Log::error("[BscTransfer] RPC错误 [{$response['error']['code']}] {$response['error']['message']}");
            return;
        }
        ...
        if (isset($response['result']) && is_string($response['result']) && str_starts_with($response['result'], '0x') && !$this->subscriptionId) {
            ...
            Log::info("[BscTransfer] 订阅成功，ID={$this->subscriptionId}");
            return;
        }
        ...
        if (isset($response['result']) && is_array($response['result']) && isset($response['result'][0])) {
            Log::info("[BscTransfer] 历史事件查询完成，共 " . count($response['result']) . " 条");
    ...
    {
        Log::error("[BscTransfer] BSC 节点连接已关闭");
...
//        print_r(['bsc',$info]);
//        $info['to'] = '0xd827078b61edfcfb42cf40555c146f4024cd96dc';
        // 仅处理收款地址属于本系统的转账
        $to = $info['to'];
        ...
        try {
            ...
            Log::error("[BscTransfer] 业务处理失败 tx={$info['tx_hash']}: " . $e->getMessage());
        }
    }
