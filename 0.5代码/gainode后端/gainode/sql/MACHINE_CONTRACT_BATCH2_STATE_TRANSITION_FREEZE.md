...
> 状态：**FROZEN CANDIDATE（候选，Owner Signoff ✅；Independent Review = CHANGES_REQUIRED，IR 629，修复中）**
> 说明：本文件为 Machine Contract 第二批的**冻结候选**。状态转移矩阵 + Event Catalog 已由 Owner 逐项裁决（2026-08-15，22 项 + 2 财务硬骨头，见本文件 §4）。IR 629 返回 6 P1 + 2 P2，已按 Owner 二次裁决修复。正式 FROZEN 前须重提 Independent Review（State Machine gate）并通过。
> 起草日期：2026-08-15
...
> 通用不变量（对所有状态机成立）：① 每个 transfer 由该实体唯一 Authoritative Writer（Service）执行；② 附带 `object_version` 乐观锁校验；③ 追加 append-only 审计事件并回写 `audit_event_id`（同事务原子）；④ 状态不可任意流转（仅本文件定义的合法出边）；⑤ 超级管理员不得绕过状态机。
...
|---|---|---|---|---|---|---|
| L1 | `pending` → `posted` | 日记账批次原子过账 | 系统 / OPS_OPERATOR | 批次内分录借贷平衡；账户余额校验通过 | 更新 `apt_accounts` 余额；追加审计事件 | 不可逆（仅经 L3 冲正） |
| L2 | `pending` → `reversed` | 入账前冲正 | OPS_OPERATOR 发起 + RISK_APPROVER 审批 | 分录尚未 posted | 追加 reversal 分录 | TRUE_TERMINAL |
| L3 | `posted` → `reversed` | 入账后冲正 | OPS_OPERATOR 发起 + RISK_APPROVER 审批 | 冲正审批通过 | 追加 reversal 分录；反向更新 `apt_accounts` | TRUE_TERMINAL |
| L4 | `pending` → `disputed` | 对账不符/异常标记 | OPS_OPERATOR / FINANCE_REVIEWER / 系统 | 对账差异记录 | 冻结标记（见 Accounting Delta Matrix） | 可逆（L6/L7） |
| L5 | `posted` → `disputed` | 入账后发现争议 | OPS_OPERATOR / FINANCE_REVIEWER / 系统 | 对账差异记录 | 冻结该笔影响 | 可逆（L6/L7） |
| L6 | `disputed` → `posted` | 仲裁确认有效 | RISK_APPROVER 裁决 | 裁决通过 | 按 origin 入账或保持 | STABLE（可再经 L3/L5） |
| L7 | `disputed` → `reversed` | 仲裁判定冲正 | RISK_APPROVER 裁决 | 裁决冲正 | 追加 reversal 分录 | TRUE_TERMINAL |
...
**Ledger Mutation Field Contract（IR 629 P1-4，方案 A）**：仅 `state`（唯一可变列）与 `audit_event_id`（state 流转证据回写）受 Authoritative Writer 控制更新；其余字段（`ledger_entry_id`/`account_id`/`asset`/`quantity`/`entry_direction`/`entry_type`/`source_object_type`/`source_object_id`/`journal_batch_id`/`reversal_of`/`idempotency_key`/`rule_version`/`snapshot_id`/`created_time`）永久 immutable。状态机更新走显式受控 update 路径（白名单两列 + object_version + transition guard），不放宽 append-only 防线。

**Accounting Delta Matrix（IR 629 P1-4）**：origin 由审计事件链确定（该分录最近一次进入 disputed 的 L4/L5 事件 before 快照）。

| 路径 | origin | L6 到 posted 账户 delta | L7 到 reversed 账户 delta |
|---|---|---|---|
| L4 | pending | +quantity 入账 | 0（未入账，仅取消标记） |
| L5 | posted | 0 保持 | -quantity 反向冲正 |

不变量：冻结期净影响恒为 0；仲裁后每个 origin 恰好一次正确余额效果，不二次入账/冲正。
...
| M4 | `open` → `locked` | 直接锁定 | OPS_OPERATOR / 系统 | 允许跳过 closing（运营兜底） | 禁止新投注 | STABLE（可 M5/M8） |
...
| M8 | `draft`/`open`/`closing`/`locked`/`awaiting_result` → `void` | 赛事取消/作废 | OPS_OPERATOR / 系统 | 四类原因（赛事取消/延期超期/数据不可用/监管） | 触发订单退款 | TRUE_TERMINAL |
...
| P4 | `settling` → `settled` | 结算完成 | 系统 | 结算计算完成 | 更新账户/ledger | STABLE（可 P7） |
| P5 | `settling` → `refunding` | 结算中 Market void 需退款 | 系统 | 仅 `Market void` 触发 | 冻结待退 | 可逆（P6） |
...
| P10 | `submitted` → `refunding` | Market void 退款（未锁定） | 系统 | 仅 `Market void` 触发 | 冻结待退 | 可逆（P6） |
| P11 | `locked` → `refunding` | Market void 退款（已锁定） | 系统 | 仅 `Market void` 触发 | 冻结待退 | 可逆（P6） |
| P12 | `awaiting_result` → `refunding` | Market void 退款（等结果中） | 系统 | 仅 `Market void` 触发 | 冻结待退 | 可逆（P6） |
...
- **void→refund 断路修复（IR 629 P1-2）**：Market void 时，结算前的任意订单状态（`submitted`/`locked`/`awaiting_result`/`settling`）均可进入 `refunding`（P5/P10/P11/P12）；退款范围 = 全额本金；`idempotency_key` 防重。
...
| O6 | `matching` → `completed` | 全部成交 | 系统 | 全部成交 | 生成 Trade + Ledger | STABLE（可 O11） |
...
| 联动 | 依据 |
|---|---|
| Market `void` → Prediction Order `submitted`/`locked`/`awaiting_result`/`settling` → `refunding` → `refunded` | 05 §4「已作废/已取消」+ IR 629 P1-2 |
...
- [ ] Ledger Mutation Field Contract（方案 A：仅 state + audit_event_id 受控可变）+ Accounting Delta Matrix 无二次入账/冲正
