...
class MakeDao extends Command
{
    ...
    protected static $defaultDescription = '一键生成Dao';
     ...
     * @throws \Exception
     */
    private function createMakeLogs(Dao $make,$table){
        $fileClass = $make->getFileClass($table);
        $data = ["type"=>'dao',"table"=>$table,'file_class'=>$fileClass,'created_time'=>getCurrentDate('unix'),'updated_time'=>getCurrentDate('unix')];
        $conn = Db::connection($this->adapter);
