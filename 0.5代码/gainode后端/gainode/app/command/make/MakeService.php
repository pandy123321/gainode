...
class MakeService extends Command
{
    ...
    protected static $defaultDescription = '一键生成Service';
    ...
    private function list(OutputInterface $output)
    {
        ...
        if(empty($tableLists)){
            $output->writeln('暂无需要创建的service类');
            return self::FAILURE;
        }else{
            $headers = ['数据库表', '文件地址'];
            $rows = [];
     ...
     * @throws \Exception
     */
    private function createMakeLogs(Service $make,$table){
        $fileClass = $make->getFileClass($table);
        $data = ["type"=>'service',"table"=>$table,'file_class'=>$fileClass,'created_time'=>getCurrentDate('unix'),'updated_time'=>getCurrentDate('unix')];
        $conn = Db::connection($this->adapter);
