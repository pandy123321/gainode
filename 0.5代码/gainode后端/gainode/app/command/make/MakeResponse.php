...
class MakeResponse extends Command
{
    /**
     * 数据库连接
     * @var string
    ...
    protected static $defaultDescription = '一键生成Response';
    ...
    {
        $make = new Response($this->adapter);
        $tableLists = $make->getList();
        if(empty($tableLists)){
            $output->writeln('暂无需要创建的response类');
            return self::FAILURE;
        }else{
            $headers = ['数据库表', '文件地址'];
            $rows = [];
     ...
     * @throws \Exception
     */
    private function createMakeLogs(Response $make,$table){
        $fileClass = $make->getFileClass($table);
        $data = ["type"=>'response',"table"=>$table,'file_class'=>$fileClass,'created_time'=>getCurrentDate('unix'),'updated_time'=>getCurrentDate('unix')];
        $conn = Db::connection($this->adapter);
