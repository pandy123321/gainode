...
SoD 必须基于 05 §8/§11 canonical Role ID（SUPPORT_AGENT/OPS_OPERATOR/KYC_REVIEWER/
  RISK_ANALYST/RISK_APPROVER/LEDGER_OPERATOR/FINANCE_REVIEWER/
  PARAM_EDITOR/PARAM_APPROVER/RELEASE_OPERATOR/AUDITOR/ADMIN_SECURITY）
不得以 UI Persona（超级管理员/运营/客服）替代 canonical 角色检查
参数流程必须验证：PARAM_EDITOR ≠ PARAM_APPROVER ≠ RELEASE_OPERATOR
```

### Gate 4: P0 + BLOCKING Contract Gap 禁止共存

```text
P0 页面不得存在未解决的 BLOCKING Contract Gap。
P0_WITH_UNRESOLVED_BLOCKING_CONTRACT_GAP = MUST_EQUAL 0
```

**验证方式**：
1. 从 `GAINODE_ADMIN_CONTRACT_GAP_REGISTER_V2.4.1.md` 的 Page→Gap JOIN 表取出所有 BLOCKING_LEVEL=BLOCKING 的页面
2. 交叉检查 `GAINODE_ADMIN_PAGE_ID_MIGRATION_MATRIX_V2.4.1.md`——这些页面的 Priority 不得为 P0
3. JOIN 表中标记为 NON_BLOCKING 的 Gap（含 BASE_CONTRACT=FROZEN 页面）不触发 Gate 4 阻断
4. Contract Status 须以 JOIN 表为唯一派生源，Migration Matrix 和 Page Map 中对应值必须一致

```text
P0_WITH_BLOCKING_CONTRACT_GAP = 0（当前状态：A-USER-004 已降为 P1_CONDITIONAL，满足 Gate 4）
```
...
G3: HIGH_RISK_SELF_APPROVAL = FORBIDDEN/VIOLATION
    违规页面数 = ___
G4: P0_WITH_BLOCKING_CONTRACT_GAP = ___（必须为 0）
